...
def automatic_extraction():
    ...
    if config['email'] != "" and config['password'] != "":
        ...
        for nombre_empresa in lista_empresas:
            ...
            print("Ahora buscamos la pestaña de Cuenta de resultados")
            
            income_activo = wait_until_found_xpath("//main/div/div/div/div/div/div/div/div/div/div/div/div/div/div/div[@class='v-tab v-tab--active' and contains(string(), 'Cuenta de resultados')]", 2)
            if income_activo is None:
                income = wait_until_found_xpath("//main/div/div/div/div/div/div/div/div/div/div/div/div/div/div/div[@class='v-tab' and contains(string(), 'Cuenta de resultados')]", 20)
                if income is not None:
                    print("Pulsamos en Cuenta de resultados")
                    income.click()
                else:
                    print("No se ha podido pulsar Cuenta de resultados")
                    exit(1)
                print("Ya estamos en Cuenta de resultados")
            ...
            # Sacamos de la página la tabla Cuenta de resultados y la guardamos en table_income
            ...
            print("Ahora buscamos la pestaña de Balance de situación")
            balance = wait_until_found_xpath("//main/div/div/div/div/div/div/div/div/div/div/div/div/div/div/div[@class='v-tab' and contains(text(), 'Balance de situación')]", 20)
            if balance is not None:
                print("Pulsamos en Balance de situación")
                balance.click()
            else:
                print("No se ha podido pulsar Balance de situación")
                exit(1)
            print("Ya estamos en Balance de situación")
            # Sacamos el código de la página
            html_balance = browser.page_source
            # Le damos un formato bonito
            soup_balance=BeautifulSoup(html_balance,'html.parser')
            # Sacamos de la página la tabla Balance de situación y la guardamos en table_balance
            ...
            print("Ahora buscamos la pestaña de Estado de flujo de efectivo")
            cash_flow = wait_until_found_xpath("//main/div/div/div/div/div/div/div/div/div/div/div/div/div/div/div[@class='v-tab' and contains(text(), 'Estado de flujo de efectivo')]", 20)
            if cash_flow is not None:
                print("Pulsamos en Estado de flujo de efectivo")
                cash_flow.click()
            else:
                print("No se ha podido pulsar Estado de flujo de efectivo")
                exit(1)
            print("Ya estamos en Estado de flujo de efectivo")
            # Sacamos el código de la página
            html_cash = browser.page_source
            # Le damos un formato bonito
            soup_cash=BeautifulSoup(html_cash,'html.parser')
            # Sacamos de la página la tabla Estado de flujo de efectivo y la guardamos en table_cash
...
def clicked():
    ...
    #name=name_entry.get() 
    if "No modificar esto" != name_entry.get():
        lbl.configure (text="TE DIJE QUE NO MODIFIQUES, PON lo que había antes para poder ejecutar GILIPOLLAS")
    else:
        button_execution()
...
class Window(tk.Tk):
    def __init__(self):
        ...
        self.title("Programa cutre hecho para Manu. INTERFAZ MEJORADA 2.0")
        self.minsize(500,400)

# Creamos la ventana del programa
window = Window()
# Añadimos una etiqueta a la ventana
lbl = tk.Label(window, text="Hola MANU, pulsa el PUTO botón porfa")
...
btn = tk.Button(window, text="Yo soy el botón", command=clicked)
...
name_entry.insert(0,"No modificar esto")
